...
پنل مدیریت ساده‌شده در `http://127.0.0.1:8000/admin/` در دسترس است. superuser به همه ابزارهای فنی دسترسی دارد؛ برای تبدیل یک کاربر موجود به «مدیر پرونده‌های پیوند» با دسترسی به مشاهده، ویرایش و حذف گیرندگان و اهداکنندگان اجرا کنید:

```bash
python manage.py grant_registry_manager USERNAME_OR_EMAIL
```

در داشبورد مدیر پرونده‌ها فقط ابزارهای اصلی گیرندگان، اهداکنندگان و اطلاعات هویتی برجسته می‌شوند. حذف تکی یا گروهی دارای صفحه تأیید و بررسی وابستگی‌های محافظت‌شده Django است و رکورد هویتی و داده‌های بالینی وابسته را نیز حذف می‌کند؛ لاگ دسترسی حساس با یک شناسه متنی غیرمسدودکننده برای ممیزی نگهداری می‌شود. ابتدا Center را بسازید و سپس از بخش «کاربران (هماهنگ‌کنندگان پیوند)» کاربر جدید ایجاد کنید. هیچ کاربر نمایشی یا رمز پیش‌فرضی ساخته نمی‌شود.
