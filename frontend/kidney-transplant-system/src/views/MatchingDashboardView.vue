      ...
      <section class="card detail-card">
        <template v-if="selectedProposal">
          ...
          <div class="abo-line">
            ...
            <strong :class="selectedProposal.compatibility === 'conditional' ? 'warn' : 'ok'">
              {{ antiLabel(selectedProposal.anti_hla_status) }}
            </strong>
          </div>

          <div class="hla-grid">
            <div v-for="locus in loci" :key="locus" class="hla-locus">
          ...
          <details class="score-details">
            ...
            <div class="score-components">
              ...
              <span>سختی cPRA <b>{{ fa(Math.round(selectedProposal.score_breakdown.cpra_difficulty)) }}</b></span>
...
const antiLabel = status => ({ clear: 'بدون mismatch', conditional: 'نیازمند Cross-Match و High-Resolution', mismatch: 'Mismatch' }[status] || 'بررسی‌نشده')
const compatibilityLabel = status => ({ compatible: 'سازگار', conditional: 'سازگار مشروط', incompatible: 'ناسازگار' }[status] || status)
