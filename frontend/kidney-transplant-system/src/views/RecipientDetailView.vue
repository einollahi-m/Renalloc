  ...
  <div v-else class="profile-page">
    ...
    <section class="profile-hero">
      ...
      <div class="hero-stats">
        ...
        <div><small>آخرین cPRA</small><strong>{{ r.cpra == null ? '—' : `${toFa(r.cpra)}٪` }}</strong><span>{{ latestCdc ? formatFaDate(latestCdc.performed_at) : 'ثبت نشده' }}</span></div>
    ...
    <div v-else-if="activeTab==='matches'" class="section-stack">
      ...
      <div v-else-if="matches.length" class="match-list"><article v-for="match in matches" :key="match.id" class="match-card"><div class="match-rank">{{ match.rank ? `#${toFa(match.rank)}` : '—' }}</div><div class="match-main"><strong>{{ donorCode(match) }}</strong><span>{{ match.donor.bloodType }}{{ match.donor.rhFactor==='positive'?'+':'-' }} · {{ match.donor.donorType==='living_related'?'زنده خویشاوند':'زنده غیرخویشاوند' }}</span><div class="tag-row"><span :class="['badge',compatibilityTone(match.compatibility)]">{{ match.compatibility_display }}</span><span :class="['badge',match.abo_compatible?'badge-success':'badge-danger']">ABO {{ match.abo_compatible?'سازگار':'ناسازگار' }}</span><span class="badge badge-info">HLA {{ toFa(match.hla_summary?.total_matches || 0) }}/{{ toFa(10) }}</span><span v-if="match.creg_summary?.has_antibody" :class="['badge',match.creg_summary?.has_potential_conflict?'badge-warning':'badge-success']">CREG {{ match.creg_summary?.has_potential_conflict?'بالقوه موجود':'یافت نشد' }}</span></div><small v-if="match.rejection_reasons?.length" class="text-danger">{{ reasonMessages(match.rejection_reasons) }}</small><small v-else-if="match.warnings?.length" class="text-warning">{{ reasonMessages(match.warnings) }}</small></div><div class="score-ring">{{ toFa(Math.round(match.final_score)) }}<small>امتیاز</small></div></article></div>
...
async function loadMatches(){matchingLoading.value=true;try{const response=await registryApi.listMatchProposals({recipient_id:route.params.id});const seen=new Set();matches.value=response.proposals.filter(item=>{if(seen.has(item.donor._id))return false;seen.add(item.donor._id);return true});serverMatchesOpen.value=true}catch(error){toast('error','خطا',errorDetail(error))}finally{matchingLoading.value=false}}
function exportMatches(){const headers=['رتبه','کد اهداکننده','گروه خونی','نوع','وضعیت سازگاری','ABO','تطابق HLA','CREG','امتیاز','هشدارها'];const rows=matches.value.map(match=>[match.rank||'',donorCode(match),`${match.donor.bloodType}${match.donor.rhFactor==='positive'?'+':'-'}`,match.donor.donorType,match.compatibility_display,match.abo_compatible?'سازگار':'ناسازگار',`${match.hla_summary?.total_matches||0}/10`,match.creg_summary?.has_potential_conflict?'بالقوه موجود':'یافت نشد',match.final_score,reasonMessages(match.warnings||[])]);exportExcelTable(`recipient-${route.params.id}-matches.xls`,'سازگاری‌ها',headers,rows)}
