  ...
  <div>
    ...
    <div class="cross-layout">
      <section class="card list-card">
        ...
        <button v-for="item in items" :key="item.id" :class="['request-row',{active:selected?.id===item.id}]" @click="selected=item;note=item.physician_note||''">
        ...
        <template v-if="selected">
          ...
          <div class="actions">
            ...
            <template v-else-if="selected.status === 'awaiting_high_resolution'">
              <button class="btn btn-primary" @click="update('negative')"><i class="ri-dna-line"></i> بررسی High-Resolution و نهایی‌سازی</button>
            </template>
...
const filter=ref('');const items=ref([]);const selected=ref(null);const note=ref('');const loading=ref(false);const saving=ref(false);const error=ref('')
